...
## 编写一个 sparksql 代码的正确步骤
### spark.local.dir 的设置
1. 该参数主要用于 spark 写中间数据，如RDD Cache，Shuffle，Spill等数据的位置
2. 可在 spark-defaults.conf 文件中设置，一般都会在代码层面设置
3. 存储在本地文件系统
### check_point 检查点设置
1. 将 rdd 的中间结果写入磁盘，默认存储在 hdfs 上
2. 该参数 ***必须*** 添加，以保证程序的高容错性
3. 设置检查点步骤
   1. 设置检查点数据存储路径：sc.setCheckpointDir("./checkpoint1")
   2. 调用检查点方法：wordRDD.checkpoint()
### sparksql 的使用技巧
1. spark.table("")，读取一个表中的数据；可以选择该表中的一些字段 spark.table("").select("","")或者spark.table("").selectExpr("")
